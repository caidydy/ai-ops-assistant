...
async def load_mcp_tools_independently(
...
) -> tuple[list[Union[BaseTool, Any]], Dict[str, str]]:
    """Load tools one MCP server at a time.

    ``MultiServerMCPClient.get_tools()`` fails as a whole when any configured
    server is unreachable.  A local optional server (for example the monitor
    demo server) must not hide the tools supplied by a healthy CLS server.
    """
    all_servers = servers or DEFAULT_MCP_SERVERS
    ...
    for name, server in all_servers.items():
        try:
            ...
            server_tools, error = await load_mcp_tools_safe(client)
            if error:
                errors[name] = error
                logger.warning(f"MCP 服务 [{name}] 工具加载失败，将跳过该服务:\n{error}")
                continue

            tools.extend(server_tools)
            logger.info(f"MCP 服务 [{name}] 成功加载 {len(server_tools)} 个工具")
        except BaseException as e:
            error = format_exception_chain(e)
            errors[name] = error
            logger.warning(f"MCP 服务 [{name}] 初始化失败，将跳过该服务:\n{error}")
