...
class AIOpsService:
    ...
    async def diagnose(
        ...
        """
        # 使用固定的 AIOps 任务描述（方案一优先走腾讯云 CLS MCP 告警工具）
        from textwrap import dedent
        aiops_task = dedent("""诊断当前系统是否存在告警，如果存在告警请详细分析告警原因并生成诊断报告。

                ## 工具使用优先级（必须遵守）
                1. **优先**使用 CLS MCP 告警相关工具获取活跃告警：
                   - DescribeAlarms（告警策略/告警列表）
                   - DescribeAlertRecordHistory / GetAlarmDetail / GetAlarmLog（告警记录与详情）
                2. 拿到告警后，用 SearchLog / DescribeLogContext 等工具查相关日志作为证据
                3. 如有监控指标工具（QueryMetric / QueryRangeMetric / query_cpu_metrics 等），用于补充症状描述
                4. **仅当**上述 CLS 告警工具均不可用时，才可回退使用 query_prometheus_alerts
                5. 禁止在未调用告警工具的情况下编造告警清单；禁止因 Prometheus 不可用就直接结束诊断
                ...
                **重要提醒**：
                ...
                - 若 CLS 返回无活跃告警，也要在报告中明确说明「当前无活跃告警」，并简述查询依据""")
